...
var t = 'Real-debrid API KEYY';
var IMDBurl = ''; //imdb API url goes here
...
var s =
  {
    ...
    headers:function(h) {  if(t) h['Authorization'] = 'Bearer ' + t; return h; },
...
function rdtorrents(){
  ...
  for (var i = list.length, a; i-- && (a = list[i]);){
    a.style.width = "85%";
    a.parentElement.innerHTML += '<a href="'+ list[i].textContent +'"></a>';
  };
  return false;
};
...
function runIMDB(){
    const imdb = document.body.querySelectorAll('a[href^="http://www.imdb.com/title/"]');
    if (isEmpty(imdb)) {
        return false;
    } else {
        for(var k in imdb) {
            imdb[k].onclick = function () {
              drop(event);
              reqIMDB(this);
            }
        };
...
function refineTrialers(el, rating, formats){
  if(isEmpty(formats)) {
    console.log("No Trailers");
  } else {
      var html = '<select class="watcht" >', formats, i;
      html += '<option value="#"> Trailer </option>';

        for(var i = 0; i < formats.length; i++) {
            html += '<option value="' + formats[i].url + '"> ' + formats[i].format + ' </option>';
        };
      ...
      let monitor = el.nextElementSibling;
      monitor.addEventListener('change', function() {
         GM_openInTab(this.value);
      }, false);
...
function reqIMDB(el) {
  let id = el.href.split('/').slice(-2).shift();
  
    GM_xmlhttpRequest({
        url: IMDBurl + id,
        method: "GET",
        onload: function(res) {
            let content = JSON.parse(res.responseText);
            let rating = isEmpty(content.data.rating) ? 'NA' : content.data.rating;
            
            let formats = [];
            if (isEmpty(content.data.trailer)){
                //console.log("No Trailers");
            } else {

                let trailers = content.data.trailer.encodings;
                
                for (var k in trailers) {
                    formats.push(trailers[k]);
                }

            };
            refineTrialers(el, rating, formats);

            
        },
        onerror: function(res) {
            console.log('Error during GM_xmlhttpRequest to ' + url + ': ' + res.statusText);
        }
    });
};
function main() {
  if (document.location.href == s.magview) {
    rdtorrents();
  };
...
function insertIcons(parent) {
  ...
  for(var i = list.length, a; i-- && (a = list[i]);) {
    insertIcon(a, onMagnetClick, 'Send 2 RD');
  }
}
...
function updateStyle() {
  ...
  style.textContent = '\
    #adh-bar { position:fixed;z-index:2147483647;top:-1px;right:5px;background-color:white;margin:0;text-align:center;font-weight:bold;font-family:sans-serif;color:black;font-size:14px;line-height:18px;text-shadow:none; }\
    ...
    strong.rating {display: inline !important; background-color: #e6b91e; font-size: 15pt !important; border: 2px solid #e6b91e; border-radius: 6px; z-index: 9999;}\
    select.watcht{display:inline-block;max-width:32pt;background-color:#015b7f;color:#fafafa;appearance:none;-webkit-appearance:none;-moz-appearance:none;overflow:hidden;}\
';
}
...
IMDBurl == '' ? '' : window.setTimeout(runIMDB, 200);
